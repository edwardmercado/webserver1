...
<link rel = "stylesheet" rev = "stylesheet" href = "userArchiveForm.css">
<script>
function showResult(str){
	...
	if(xhr){
		...
		xhr.open("GET", "showArchiveUser.php?q="+str, true);
		xhr.send(null);
	}
	else{
...
<div class = "menuList" id = "display">
	<?php
		...
		$sql = $db->query("SELECT * from tbl_user WHERE id != '$id' AND archived = 'yes' ORDER BY id ASC");
		...
		if($sql){
			while($rows = $db->fetch_array($sql)){
				...
				<td><a href = "restoreUser.php?id=<?php echo $id ?>"><img src="Images/restore.png" style = "width:70px; height:24px;"></a></td></tr>
				<?php 
			}
		}
	?>
